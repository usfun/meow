...
// 播放器功能配置
var mkPlayer = {
    api: siteinfo.url + "libs/XMusic.api.php", // api地址
...
function orderChange() {
    ...
    switch(rem.order) {
        case 1:     // 单曲循环 -> 列表循环
            orderDiv.html('<span class="iconfont icon-liebiaoxunhuan"></span>');
        ...
        case 3:     // 随机播放 -> 单曲循环
            orderDiv.html('<span class="iconfont icon-danquxunhuan"></span>');
        ...
        default:    // 列表循环(其它) -> 随机播放
            orderDiv.html('<span class="iconfont icon-suiji"></span>');
...
function audioPlay() {
    ...
    $(".btn-play").html('<span class="iconfont icon-bofang1-copy"></span>');        // 恢复暂停
    ...
    var msg = "" + music.name + " - " + music.artist;  // 改变浏览器标题
...
function audioPause() {
    ...
    $(".btn-play").html('<span class="iconfont icon-bofang"></span>');      // 取消暂停
    ...
    switch (rem.order ? rem.order : 1) {
        ...
        case 3: 
            if (musicList[1] && musicList[1].item.length) {
                ...
                var position = id * 50;
                playList(id);
                $("#main-list").animate({scrollTop: position}, 500);
            }
        break;
        default:
            playList(rem.playid + 1); 
...
console.info('欢迎使用 MKOnlinePlayer-慕乔特制版!\n当前版本：'+mkPlayer.version+' \n作者：mengkun(https://mkblog.cn)\n当前主题：慕乔小清新(https://leeleo.cn)\n歌曲来源于各大音乐平台\nGithub：https://github.com/mengkunsoft/MKOnlineMusicPlayer');
