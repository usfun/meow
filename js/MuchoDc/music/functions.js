...
$(function(){
    ...
    $("#sheet").on("click",".sheet-cover,.sheet-name", function() {
        ...
        if(musicList[num].item.length === 0 && musicList[num].creatorID) {
            layer.msg('列表读取中...'); // 0代表加载的风格，支持0-2
            // ajax加载数据
   ...
   // 点击同步云音乐
    $("#sheet").on("click",".login-in", function() {
        layer.prompt({
            title: "请输入您的网易云 UID",
            closeBtn: !1,
            btn: ['<span class="iconfont icon-edit" style="font-size:15px;font-weight:800;"></span> 确定', '<span class="iconfont icon-logout" style="font-size:15px;font-weight:800;"></span> 取消', '<span class="iconfont icon-profile" style="font-size:15px;font-weight:800;"></span> 帮助'],
            btn3: function(e, i) {
                layer.open({
                    title: "如何获取您的网易云UID？",
                    closeBtn: !1,
                    btn: ['<span class="iconfont icon-logout" style="font-size:15px;font-weight:800;"></span> 知道啦'],
                    area: "280px;",
                    shade: .6,
                    anim: -1,
                    content: '1、首先<a href="http://music.163.com/" target="_blank" class="js-leeleo-color-cur">点我(http://music.163.com/)</a>打开网易云音乐官网<br>2、然后点击页面右上角的“登录”，登录您的账号<br>3、点击您的头像，进入个人中心<br>4、此时浏览器地址栏 <span class="js-leeleo-color-cur">/user/home?id=</span> 后面的<span  class="js-leeleo-color-cur">数字</span>就是您的网易云 UID'
                })
            }
        },
    ...
    // 图片加载失败处理
    // $('img').error(function(){
    //     $(this).attr('src', siteinfo.url+'asset/img/MuchoDc/music/f-leeleo.jpg');
    // });
...
function musicInfo(list, index) {
    ...
    var tempStr = '<div  class="js-music-color" style="padding:10px 20px;"><span class="info-title">歌名：</span>' + music.name + 
    ...
    '<span style="margin-left: 10px" class="info-btn" onclick="thisShare(this)" data-list="' + list + '" data-index="' + index + '">&nbsp;外链</span></div>';
    ...
    layer.open({
        ...
        id: 'LAY_layuipro',
        resize: false,
        btn: ['<span class="iconfont icon-logout" style="font-size:15px;font-weight:800;"></span> 关闭'],
        btnAlign: 'c',
...
// 展现搜索弹窗
function searchBox() {
    var tmpHtml = '<form onSubmit="return searchSubmit()" id="onlysub"><div id="search-area">' + 
    ...
    layer.open({
        ...
        ,area: '280px;'
        ,shade: 0.6 //遮罩透明度
        ,btn: ['<span class="iconfont icon-logout" style="font-size:15px;font-weight:800;"></span> 关闭','<span class="iconfont icon-search2" style="font-size:15px;font-weight:800;"></span> 搜索']
...
function loadList(list) {
    ...
    if(musicList[list].item.length == 0) {
        if(musicList[0].item.length == 0){addListbar("nosou");}
        else{
...
function addListbar(types) {
    ...
    switch(types) {
        ...
        case "nosou":  // 列表中没有内容
            html = '<div class="list-item text-center" id="list-foot">没有搜索记录哦，快去搜一个！</div>';
        break;
...
function dataBox(choose) {
    $('#btn-area .active').removeClass('active');
    switch(choose) {
        case "list":    // 显示播放列表
